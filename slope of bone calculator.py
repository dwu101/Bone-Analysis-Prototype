...
def find_edge(x,y):
    ...
    while True: # finds the edge of the bone 
        ...
        elif (img[y,x][0] > 150 and img[y,x][1] > 150 and img[y,x][2] > 150).any():
            if (img[y,x-1][0] > 150 and img[y,x-1][1] > 150 and img[y,x-1][2] > 150).any():
...
def edge_finder(xp, image_name):
    ...
    for i in range(1,150):
        ...
        append = find_edge(x-1, y//xp + ((-1)**i) * i)
        if append == "None":
            append = find_edge(x-1,y//xp +((-1)**(i+1)*i))
    ...
    while True:
        try:
            if math.fabs(edge_points[x][1] - edge_points[x-1][1]) > 5 : 
                del edge_points[x]
            else:
                x+=1
...
def slope(points): #this doesnt work for 10 cuz the edge is straight. figure out a work around.
...
def angle_calculator(num1, num2):
    ...
    angle_rad = math.acos((line1_len**2 + line2_len**2 - line3_len**2)/(2*line2_len*line1_len))
...
# print(angle_calculator(bottom_slope, top_slope))
